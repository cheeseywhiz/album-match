...
def login_with_discogs():
    ...
    discogs = get_discogs()
    redirect_url = flask.url_for('discogs_auth.discogs_oauth_callback', vitals_callback=vitals_callback, _external=True)
...
def discogs_oauth_callback():
    ...
    if error is not None and not is_discogs_authenticated():
        return flask.redirect(flask.url_for('discogs_auth.my_discogs_auth_error_page', error=error,
