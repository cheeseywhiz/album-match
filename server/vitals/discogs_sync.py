...
def plan_to_download_album_cover(collection, catalog):
    ...
    # TODO: use proper static filesystem folder path
    file_location = f'/srv/vitals/static/{static_path}'
    file_url = f'/static/{static_path}'
    return (lambda descriptor: plan_to_set_album_cover(catalog, file_url, descriptor))(None), dict(
...
def plan_to_set_album_cover(catalog, file_url, descriptor):
    # TODO: set file_url
    return 'UPDATE albums SET descriptor = %s WHERE catalog = %s;', (descriptor, catalog)
...
def get_user_synced_albums(username):
    return {
        row.catalog
        for row in db.get_db().execute('SELECT catalog FROM collections WHERE username = %s;', (username, ))
    }
...
def get_all_synced_albums():
    return {
        row.catalog
        for row in db.get_db().execute('SELECT catalog FROM albums;')
    }
...
def discogs_sync_plan():
    ...
    return utils.jsonify()(
        add_collection=add_collection, rm_collection=rm_collection, error_messages=error_messages,
    )
